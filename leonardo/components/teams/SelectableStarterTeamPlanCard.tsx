...
const STARTER_COST_PER_SEAT = TEAM_PLANS_COST_PER_SEAT.STARTER ?? 24;

const STARTER_FEATURES: (string | [string, string])[] = [
  [
    `${(25000).toLocaleString()}  Fast Tokens Per Seat`,
    "Resets on your Billing Date",
  ],
  `${(75000).toLocaleString()} Rollover Token Bank Capacity Per Seat`,
  ...
  return (
    ...
    >
      <TeamPlanCardHeader
        ...
        planName="Starter"
        planNameStyle="apprentice"
        pricePerSeat={STARTER_COST_PER_SEAT}
      ...
      >
        {STARTER_FEATURES.map((feature, idx) => (
          <FeatureItem
            key={idx}
            icon={
              <CheckCircleOutlineIcon className="text-highlight-foreground text-secondary-foreground h-4 w-4 shrink-0" />
            }
            text={feature}
          />
        ))}
